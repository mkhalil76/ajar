	...
	<div class="row">
      ...
      <div class="card">
        <div class="card-body col-md-8">
          <h4 class="card-title">تعديل الفرع</h4>
          {!! Form::open(['url' => 'branchs/update-branch', 'files' => true, 'method' => 'POST']) !!}
          <!--<form class="forms-sample" enctype="multipart/form-data" method="POST" action="{{ route('providers.upload-documents') }}">-->
            ...
            <input type="hidden" value="{{$branch->id}}" name="branch_id"/>
                ...
                <div class="col-sm-7">
                    <select class="js-example-basic-single" name="city" id="city_id" style="width:100%" required>
                      <option value="" > إختر المدينة </option>
                      @foreach($city as $c)
                            <option value="{{$c->id}}" {{ ($branch->city == $c->name)?'selected':'' }} >{{ $c->name }}</option>
                      @endforeach
                    </select>
                    @if ($errors->has('city'))
                       <strong>
              ...
              <div class="form-group row">
                  ...
                  <div class="col-sm-7">
                    <select class="js-example-basic-single"  name="location_id" id="locations_id" style="width:100%" required>
                        <option value="" > إختر الموقع </option>
                        @foreach($locations as $location)
                              <option value="{{$location->id}}" {{ ($branch->location_id == $location->name)?'selected':'' }} >{{ $location->name }}</option>
                        @endforeach
                    </div>
                </div>
                <div class="form-group row">
                    ...
                    <div class="col-sm-7">
                    <input type="text" value="{{ $branch->branch_mobile}}" name="branch_mobile" class="form-control" id="branch_mobile_id" required/>
                    </div>
            ...
            <button type="submit" class="btn btn-success mr-2" id="save-button">تعديل</button>
          {!! Form::close() !!}
        </div>
    ...
    <script>
        $(document).ready(function () {
            $('#city_id').change(function () {
                $('#locations_id').find('option')
                    .remove()
                    .end()
                    .append("<option value=''>إختر الموقع</option>");
